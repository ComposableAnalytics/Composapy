...
import System.Net
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.Services\bin\Debug\CompAnalytics.Services.dll')
...
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.IServices\bin\Debug\CompAnalytics.IServices.dll')
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.Core\bin\Debug\CompAnalytics.Core.dll')
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.Extension.Tables\bin\Debug\CompAnalytics.Extension.Tables.dll')
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.Extension.Tables\bin\Debug\CompAnalytics.Extension.dll')
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.Extension.Spreadsheet\bin\Debug\CompAnalytics.Extension.Spreadsheet.dll')
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\UnitTests\bin\Debug\UnitTests.dll')
clr.AddReference(r'C:\Users\Mike\source\Workspaces\Composable Analytics\Product\CompAnalytics.Extension.Tableau\bin\Debug\Microsoft.VisualStudio.QualityTools.UnitTestFramework.dll')
...
class ComposableSession:
    ...
    def __init__(self, username_or_api, password = None):
        ...
        for method in self.ResourceManager.AvailableServices():
            ...
            try:
                self._services[service_string] = self.ResourceManager.CreateAuthChannel[method](service_string)
            except: 
                self._services[service_string] = self.ResourceManager.CreateAuthChannelNoWebScripting[method](service_string)
    ...
    def get_queryview(self,id):
        ...
        dtypes_dict = self._make_pandas_dtypes_from_list_of_column_defs(queryview_data.ColumnDefinitions)
    ...
    def run_dataflow(self,app_id):
        ...
        execution_context = appservice.CreateExecutionContext(app, Contracts.ExecutionContextOptions())
    ...
    def wait_for_run_execution(self,run_id):
        ...
        output = {}
        output['execution_status'] = execution_names[self._services['ApplicationService'].GetRun(run_id).Status]
    ...
    def queryview_from_id_direct(self,queryview_id):
        ...
        for column_definition in columns_definitions:
            if not column_definition.Exclude:
                ...
                column_dtypes[column_definition.Name] = MAP_CS_TYPES_TO_PANDAS_TYPES[column_definition.Type]
    ...
    def queryview_from_id(self,queryview_id):
        ...
        stream = self._services['QueryViewService'].GetQueryResultsDownloadWeb(queryview,'csv')
    ...
    def convert_table_to_pandas(self,table):
        table_results = self._services['TableService'].GetResultFromTable(table,0,0x7FFFFFFF)
    ...
    def _upload_file(self,file_path, execution_context):   
        ...
        print(dir(self.ResourceManager))
        uploadResult = FileUploadClient.UploadFile(self.ResourceManager.Login,execution_context, fileUploader.UiHandle, fileInput.UiHandle, filePath)
        return uploadResult
    ...
    def run_dataflow_with_inputs(self,app_id,dict_of_inputs):
            ...
            if 'External' in module.ModuleType.Name:
                ...
                if inpt.ValueObj in dict_of_inputs.keys():
                    ...
                    if module.ModuleType.Name in method_to_call.keys():
                        method_to_call[module.ModuleType.Name](module,dict_of_inputs[inpt.ValueObj])
        ...
        execution_context = self._services['ApplicationService'].CreateExecutionContext(app, Contracts.ExecutionContextOptions())
        ...
        for i in range(len(app.Modules)):
            if app.Modules[i].ModuleType.Name == "External File Input":
                ...
                fileInputName = next(inpt for inpt in fileUploader.ModuleInputs if inpt.Name == 'Name')

                if fileInputName.ValueObj in dict_of_inputs.keys():
                    fileInput = next(inpt for inpt in fileUploader.ModuleInputs if inpt.Name == 'Input')
                    ...
                    uploadResult = FileUploadClient.UploadFile(uri, self.ResourceManager.Login,execution_context, fileUploader.UiHandle, fileInput.UiHandle, filePath)
                    #print(uploadResult)
        ...
        for i in range(len(app.Modules)):
            if app.Modules[i].ModuleType.Name == "External Table Input":
                ...
                if fileInputName.ValueObj in dict_of_inputs.keys():
                    ...
                    table = Extension.Tables.CreateTableExecutor.CreateTablePython(headers,rows,types,execution_context)
                    
                    
                    ComposableSession._input_default(module,table)
        
        
        app_ran = self._services['ApplicationService'].RunExecutionContext(execution_context)
